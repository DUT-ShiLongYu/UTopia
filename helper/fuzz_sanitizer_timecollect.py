...
def run_fuzz_test(executable, corpus_dir, info_dir, max_total_time):
    ...
    try:
        ...
        subprocess.run([
            executable, 
            corpus_dir,  
            f"-max_total_time={max_total_time}",  # 运行的最大时间
            "-print_final_stats=1", 
        ...
        elapsed_time = end_time - start_time  # 计算实际运行时间
        logging.info(f"Executable: {executable}, Actual Run Time: {elapsed_time:.2f} seconds")
        print(f"Executable: {executable} ran for {elapsed_time:.2f} seconds")
