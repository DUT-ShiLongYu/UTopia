...
def periodic_backup(corpus_dir, backup_dir, interval, pause_event, backup_times, stop_backup_event):
    i = 1
    ...
    print("Backup thread terminated.")
...
def fuzz_executable(executable, result_path, fuzz_path, max_total_time, cpu_core):
    ...
    stop_backup_event = threading.Event()  # 添加终止事件

    backup_thread = threading.Thread(
        target=periodic_backup,
        args=(corpus_dir, backup_dir, 60*10, pause_event, backup_times, stop_backup_event),
        daemon=True
    )
    backup_thread.start()
...
def main():
    ...
    parser.add_argument("--max_total_time", type=int, default=60*60, help="Max fuzzing time in seconds")
