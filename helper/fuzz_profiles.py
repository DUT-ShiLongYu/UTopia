...
def generate_coverage_report(profile_path, target, group_name):
    # 查找 profile_path 下的所有 _{group_name}.profraw 文件
    profraw_files = list(profile_path.glob(f"*_{group_name}.profraw"))
    ...
    # 生成覆盖率报告
    coverage_report_file = profile_path / f"{target}_coverage_{group_name}.txt"
...
def profile_executable(executable, profile_path, target, group_name):
    # 为当前可执行文件创建corpus和info目录
    corpus_path = Path(f"/root/UTopia/exp/{target}/output/fuzzers/backup")
    corpus_dir = corpus_path / f"corpus_{executable.stem}/{group_name}"

    # 创建崩溃信息目录
    info_dir = profile_path / f"information_{executable.stem}/{group_name}"
    info_dir.mkdir(parents=True, exist_ok=True)

    profraw_file = profile_path / f"{executable.stem}_{group_name}.profraw"

    run_profile_test(str(executable), str(corpus_dir), str(info_dir), str(profraw_file))


def execute_profile_for_all_executables(target, group_name):
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(profile_executable, executable, profile_path, target, group_name) for executable in executables]
        # 等待所有并行任务完成
        for future in futures:
            future.result()  # 检查任务执行结果

    generate_coverage_report(profile_path, target,group_name)
...
def main():
    ...
    for group_name in ["1", "2"]:
        execute_profile_for_all_executables(args.target, group_name)
