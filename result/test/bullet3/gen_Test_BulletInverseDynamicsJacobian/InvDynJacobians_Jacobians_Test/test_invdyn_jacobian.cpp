...
*  .:: .:::            ::::...     .::.
*  .:: .:::      ...   .....::     .::.  Base UT: InvDynJacobians_Jacobians_Test
*  .:: .:::      .::.  ..::::.     .::.
...
extern int autofuzz36;
extern int autofuzz71;
extern btScalar autofuzz72;
extern int autofuzz77;
#ifdef __cplusplus
}
#endif
						   ...
						   double* max_error)
{
	...
	for (int loop = 0; loop < nloops; loop++)
	{
		...
		for (int idx = autofuzz22; idx < nbodies; idx++)
		{
			vec3 tmp1, tmp2;
			vec3 diff;
							...
							double* max_error)
{
	...
	for (int loop = 0; loop < nloops; loop++)
	{
		...
		for (int idx = autofuzz36; idx < nbodies; idx++)
		{
...
TEST(InvDynJacobians, Jacobians)
{
...
#endif
	const int kNumLoops = 20;
	for (int level = autofuzz71; level < kNumLevels; level++)
	{
		const int nbodies = BT_ID_POW(autofuzz72, level);
		CoilCreator coil(nbodies);
		double error;
	...
	int kMaxRandomBodies = autofuzz77;
	for (int loop = 0; loop < kRandomLoops; loop++)
	{
		RandomTreeCreator random(kMaxRandomBodies);
		double error;
...
#endif
void enterAutofuzz() {
  class AutofuzzTest : public InvDynJacobians_Jacobians_Test {
  public:
    void runTest() {
      try {
        SetUpTestCase();
