...
*  .:: .:::            ::::...     .::.
*  .:: .:::      ...   .....::     .::.  Base UT: InvDynJacobians_VelocitiesFromJacobians_Test
*  .:: .:::      .::.  ..::::.     .::.
...
extern "C" {
#endif
extern int autofuzz55;
extern int autofuzz80;
#ifdef __cplusplus
}
#endif
									...
									double* max_error)
{
	...
	for (int loop = 0; loop < nloops; loop++)
	{
		...
		for (int idx = autofuzz55; idx < nbodies; idx++)
		{
			vec3 vel1;
			vec3 omg1;
			vec3 vel2;
...
{
	const int kRandomLoops = 20;
	int kMaxRandomBodies = autofuzz80;
	const int kNumLoops = 20;
#ifdef B3_USE_DOUBLE_PRECISION
	const double kMaxError = 1e-12;
#else
...
#endif
void enterAutofuzz() {
  class AutofuzzTest : public InvDynJacobians_VelocitiesFromJacobians_Test {
  public:
    void runTest() {
      try {
        SetUpTestCase();
